...
const APPROVER_SHARE_INDEX = new Map([
  ["sander", 3],
  ["thomas", 4]
]);
...
export function decodePhoneSharePackageV1(pkg) {
  ...
  if (!APPROVER_SHARE_INDEX.has(pkg.approver_id)) {
    throw new Error("unsupported approver_id");
  }
  ...
  if (APPROVER_SHARE_INDEX.get(pkg.approver_id) !== pkg.share_index) {
    throw new Error("approver_id does not match phone share index");
  }
